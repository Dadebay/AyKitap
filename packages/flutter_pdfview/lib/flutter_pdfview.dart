...
class PDFView extends StatefulWidget {
  ...
  /// Scales every page to the view individually (Android only).
  ///
  /// When false — AndroidPdfViewer's own default — pages are scaled
  /// *relative to the largest page in the document*, so a book whose pages
  /// aren't all the same size renders its smaller pages shrunken, with
  /// bands down either side, instead of filling the width like the big
  /// ones. That is the usual result of promo/insert pages being appended to
  /// a book at a different page size.
  ///
  /// Upstream declares this parameter but never forwards it to the native
  /// side (the `.fitEachPage(...)` line in FlutterPDFView.java is commented
  /// out), which is why it is wired through in this vendored copy.
  final bool fitEachPage;

  /// Gap drawn between pages, in logical pixels. Defaults to none.
  ///
  /// On Android this is AndroidPdfViewer's `spacing`. On iOS it is PDFKit's
  /// `pageBreakMargins`, which upstream doesn't expose at all — without it
  /// there is no way to close the default inter-page gap in continuous
  /// scroll mode there.
