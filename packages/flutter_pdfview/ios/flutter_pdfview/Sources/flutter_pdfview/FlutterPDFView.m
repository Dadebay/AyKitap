...
- (instancetype)initWithFrame:(CGRect)frame
    ...
    if (document == nil) {
        ...
        // Gap drawn between pages. PDFKit's default pageBreakMargins is
        // { 4.75, 4.0, 4.75, 4.0 }, so there is always a visible band
        // between pages in continuous scroll mode unless it is overridden —
        // and upstream exposes no way to do that. (`autoSpacing`, the flag
        // that controls the gap on Android, is wired to `autoScales` here
        // instead, so it can't serve double duty.) Zero collapses the gap
        // entirely via displaysPageBreaks, which the header documents as
        // forcing the margins to 0 on its own.
        CGFloat pageSpacing = [args[@"pageSpacing"] doubleValue];
        if (pageSpacing <= 0.0) {
            _pdfView.displaysPageBreaks = NO;
        } else {
            _pdfView.displaysPageBreaks = YES;
            // Split across the facing edges so two adjacent pages add up to
            // the requested gap rather than twice it.
            _pdfView.pageBreakMargins = UIEdgeInsetsMake(pageSpacing / 2.0, 0.0, pageSpacing / 2.0, 0.0);
        }
