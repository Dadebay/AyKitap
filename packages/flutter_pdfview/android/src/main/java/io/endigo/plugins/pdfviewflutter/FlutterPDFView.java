...
//        Constants.Cache.THUMBNAILS_CACHE_SIZE = 0;
        ...
        if (config != null) {
            config
                    ...
                    // Upstream leaves this commented out, which silently
                    // pins every document to AndroidPdfViewer's default of
                    // false: smaller pages are then scaled relative to the
                    // largest page in the book rather than to the view, so a
                    // book with mixed page sizes renders its normal pages
                    // shrunken with bands down either side.
                    .fitEachPage(getBoolean(params, "fitEachPage"))
                    // Gap between pages, in dp. AndroidPdfViewer's own
                    // default is 0; exposing it lets the gap be closed (or
                    // widened) from Dart. Read as a float (Dart sends a
                    // double, which getInt's hard (int) cast would throw on)
                    // and rounded to the int dp this API takes.
                    .spacing(getSpacing(params))
    ...
    private int getSpacing(Map<String, Object> params) {
        Float spacing = getFloat(params, "pageSpacing");
        return spacing == null ? 0 : Math.round(spacing);
    }
